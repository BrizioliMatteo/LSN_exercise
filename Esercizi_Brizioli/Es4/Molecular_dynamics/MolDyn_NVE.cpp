...
void Input(void){ //Prepare all stuff for the simulation
  ...
  Epot.open("Output/ave_epot_"+nomefile+".out");
  Ekin.open("Output/ave_ekin_"+nomefile+".out");
  Etot.open("Output/ave_etot_"+nomefile+".out");  
  Temp.open("Output/ave_temp_"+nomefile+".out");
  Pres.open("Output/ave_pres_"+nomefile+".out");
	Epot.close();
  Ekin.close();
  Temp.close();
  Etot.close();
...
void Averages(int iblk) //Print results for current block
{
 ...
 /*  if(Argon==1 || Kripton==1){
 		   Epot<<setprecision(wd)<<iblk<<"\t"<<glob_av[iv]/double(iblk)*eps<<"\t"<<errore[iv]*eps<<endl; 		
			 Ekin<<setprecision(wd)<<iblk<<"\t"<<glob_av[ik]/double(iblk)*eps<<"\t"<<errore[ik]*eps<<endl;
  		 Etot<<setprecision(wd)<<iblk<<"\t"<<glob_av[ie]/double(iblk)*eps<<"\t"<<errore[ie]*eps<<endl;
  		 Temp<<setprecision(wd)<<iblk<<"\t"<<glob_av[it]/double(iblk)*e_kb<<"\t"<<errore[it]<<endl;
   		 Pres<<setprecision(wd)<<iblk<<"\t"<<glob_av[pr]/double(iblk)*eps/pow(sigma,3.)<<"\t"<<errore[pr]*eps/pow(sigma,3.)<<endl;
		}else{*/
  		 Epot<<setprecision(wd)<<iblk<<"\t"<<glob_av[iv]/double(iblk)<<"\t"<<errore[iv]<<endl; 				 Ekin<<setprecision(wd)<<iblk<<"\t"<<glob_av[ik]/double(iblk)<<"\t"<<errore[ik]<<endl;
