...
int main()
{ 
  system( "rm output.XYZ.dat output.distr.dat");
...
void Equilibration(void){
    ...
    for(int i=1;i<N_equ;i++){
      ...
      else        p= exp(2.*(rold-rnew))*znew/zold*znew/zold;
...
void Move(void){
  double xold, yold, zold, xnew, ynew, znew, p, rnew, rold, alpha;
   ofstream ConfXYZ;
   ConfXYZ.open("output.XYZ.dat",ios::app);
  //Old
    xold = x;
    yold = y;
    zold = z;
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Distr;
    ...
    Distr.open("output.distr.dat",ios::app);
