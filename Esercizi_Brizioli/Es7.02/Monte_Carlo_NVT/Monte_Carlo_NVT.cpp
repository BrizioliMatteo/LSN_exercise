...
//Read initial configuration
	...
	//faccio in modo che mi salvi solo l'ultima simulazione
  ofstream Gofr, Gave, Epot, Pres;
  Epot.open("Output/output.epot_"+nomefile+".0");
  Pres.open("Output/output.pres_"+nomefile+".0");
  Gofr.open("Output/output.gofr_"+nomefile+".0");
	Epot.close();
	Pres.close();
	Gofr.close();
...
void Averages(int iblk) //Print results for current block
{
   ...
   double delta_v,r;
    ...
    Epot.open("Output/output.epot_"+nomefile+".0",ios::app);
    Pres.open("Output/output.pres_"+nomefile+".0",ios::app);
    Gofr.open("Output/output.gofr_"+nomefile+".0",ios::app);
    Gave.open("Gofr/output.gave_"+nomefile+".0");
    
    stima_pot = blk_av[iv]/blk_norm/(double)npart + vtail; //Potential energy
    glob_av[iv] += stima_pot;
    glob_av2[iv] += stima_pot*stima_pot;
    err_par[iv]=Error(glob_av[iv],glob_av2[iv],iblk);
    
    stima_pres = rho * temp + (blk_av[iw]/blk_norm + ptail * (double)npart) / vol; //Pressure
    glob_av[iw] += stima_pres;
    glob_av2[iw] += stima_pres*stima_pres;
    err_par[iw]=Error(glob_av[iw],glob_av2[iw],iblk);
    ...
    for (int k=igofr; k<igofr+nbins; ++k){
      ...
      err_par[k]=Error(glob_av[k],glob_av2[k],iblk);

      Gofr<<setprecision(wd)<<iblk<<"\t"<<r<<"\t"<<gdir<<"\t"<<glob_av[k]/double(iblk)<<"\t"<<err_par[k]<<endl;
      if(iblk==nblk){
          Gave<<setprecision(wd)<<r<<"\t"<<glob_av[k]/double(iblk)<<"\t"<<err_par[k]<<endl;
      }
    }


//Potential energy per particle
    Epot << setw(wd) << iblk <<  setw(wd) << stima_pot << setw(wd) << glob_av[iv]/(double)iblk << setw(wd) << err_par[iv] << endl;
//Pressure
    Pres << setw(wd) << iblk <<  setw(wd) << stima_pres << setw(wd) << glob_av[iw]/(double)iblk << setw(wd) << err_par[iw] << endl;
