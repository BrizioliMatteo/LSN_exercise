...
double variationalWaveFunction_second(double v)
{
	...
	return -1./(sigma*sigma)*exp(-0.5*((v-mu)*(v-mu))/(sigma*sigma))-1./(sigma*sigma)*exp(-0.5*((v+mu)*(v+mu))/(sigma*sigma))+pow(v-mu,2.)/pow(sigma,4.)*exp(-0.5*((v-mu)*(v-mu))/(sigma*sigma))+pow(v+mu,2.)/pow(sigma,4.)*exp(-0.5*((v+mu)*(v+mu))/(sigma*sigma));
}
